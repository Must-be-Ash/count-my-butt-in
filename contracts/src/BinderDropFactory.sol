...
contract BinderDropFactory {
  ...
  function createBinderDrop(address creator, string memory defaultUri, uint cost, uint256 batchSize) external returns (address) {
    require(creator != address(0));
    address clone = Clones.clone(binderDrop);
    BinderDrop(clone).initialize(creator, defaultUri, server, cost,batchSize);
