...
contract BinderDrop is ERC721, RecoverTokens {
    ...
    event AutographBatchCreated(address minter, string orderId, address[] recipients, uint256 tokenId, bytes32 hash);
    ...
    function mintToBatch(string memory orderId, address[] memory recipients, bytes memory signature, string[] memory uris, bytes32 nonce) public payable {
      require(nonceMapping[nonce] == false, "Nonce already used");
      ...
      emit AutographBatchCreated(msg.sender, orderId, recipients, tokenCount, hash);
    }
