...
export async function overlayImages({
  ...
  try {
    ...
    const autographResizer = (await Jimp.read(autograph)).resize(800, 800);
    const backgroundImage = await fetchImageFromUrl(url);

    // Create a gradient image (adjust parameters as needed)
    const gradientOverlay = (
      await Jimp.read(Buffer.from(gradient.split(",")[1], "base64"))
    ).resize(800, 800);
    ...
    const output = await (
      ...
      .resize(800, 800)
      .composite(gradientOverlay, 0, 0, {
        mode: Jimp["BLEND_SOURCE_OVER"],
        opacityDest: 1,
        opacitySource: 1,
      })
      .composite(autographResizer, 0, 0, {
